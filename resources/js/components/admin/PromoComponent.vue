<template>
	<v-app id="inspire">
	    ...
	    :headers="headers" :options.sync="options" :server-items-length="promo.total" :items="promo.data" show-select @input="selectAll" :footer-props="footerProps">
	        <template v-slot:top>
	            <v-toolbar flat>
    ...
    export default {
        data: () => ({
        	...
        	footerProps: {
				...
				itemsPerPageText: 'promo Per Page',
	    ...
	    watch: {
		    ...
		    options: {
		    	handler(e) {
					...
					axios.get(`/api/promo`, {params: {'page': e.page,'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
					.then(res => {
	    ...
	    methods: {
	    	uploadPhoto(item) {
	    		if (this.editedItem.photo != null) {
			        const index = this.promo.data.indexOf(item);
			        let formData = new FormData();
			        formData.append( "photo", this.editedItem.photo, this.editedItem.photo.name );
			        formData.append("promo", item.id);
			        axios.post("/api/promo/photo", formData)
			    	.then(res => {
			    		this.promo.data[index].photo = res.data.promo.photo;
	    	...
	    	updateRole(item) {
	    		const index = this.promo.data.indexOf(item);
		      	axios.post("/api/promo/role", { role: item.role, promo: item.id })
		        .then(res => {
		          	this.text = res.data.promo.name + "'s Role Updated to " + res.data.promo.role;
		          	this.snackbar = true;
		        })
		        .catch(error => {
		          	this.text = error.response.data.promo.name + "'s Role Cannot Be Updated to " + error.response.data.promo.role;
		          	this.promo.data[index].role = error.response.data.promo.role;
		          	this.snackbar = true;
		        });
	    	},
	    	selectAll(e) {
	    	...
	    	deleteAll() {
		        ...
		        if(decide) {
			        ...
			        //axios.post('/api/promo/delete', {'promo': this.selected})
			        axios.post('/api/promo/delete', {'promo': selected_id})
			        .then(res => {
		        		this.text = "Records Deleted Successfully!";
			        	this.selected.map(val => {
			        		const index = this.promo.data.indexOf(val)
							this.promo.data.splice(index, 1)
			        	})
			        	this.snackbar = true;
			        }).catch(err => {
		    ...
		    editItem (item) {
		        this.editedIndex = this.promo.data.indexOf(item)
		    ...
		    save () {
		        if (this.editedIndex > -1) {
		        	...
		        	axios.put('/api/promo/' + this.editedItem.id, this.editedItem)
		        	.then(res => {
		        		this.text = "Record Updated Successfully!";
		        		this.snackbar = true;
		        		Object.assign(this.promo.data[index], res.data.promo)
		        	})
		        	.catch(err => {
		        		console.log(err.response)
			        ...
			        axios.post('/api/promo', this.editedItem)
			    	.then(res => {
		        		this.text = "Record Added Successfully!";
		        		this.snackbar = true;
			    		this.promo.data.push(res.data.promo)
			    	})
			    	.catch(err => {
		        		console.log(err.response)
