<template>
	<v-app id="inspire">
	    ...
	    :headers="headers" :options.sync="options" :server-items-length="store.total" :items="store.data" show-select @input="selectAll" :footer-props="footerProps">
	        <template v-slot:top>
	            <v-toolbar flat>
	                ...
	                <v-dialog v-model="dialog" max-width="500px">
	                    <template v-slot:activator="{ on }">
	                        <v-btn color="primary" dark class="mb-2" v-on="on">Add New store</v-btn>
    ...
    export default {
        data: () => ({
        	...
        	footerProps: {
				...
				itemsPerPageText: 'store Per Page',
	    ...
	    watch: {
		    ...
		    options: {
		    	handler(e) {
					...
					axios.get(`/api/store`, {params: {'page': e.page,'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
					.then(res => {
	    ...
	    methods: {
	    	uploadPhoto(item) {
	    		if (this.editedItem.photo != null) {
			        const index = this.store.data.indexOf(item);
			        let formData = new FormData();
			        formData.append( "photo", this.editedItem.photo, this.editedItem.photo.name );
			        formData.append("store", item.id);
			        axios.post("/api/store/photo", formData)
			    	.then(res => {
			    		this.store.data[index].photo = res.data.store.photo;
	    	...
	    	updateRole(item) {
	    		const index = this.store.data.indexOf(item);
		      	axios.post("/api/store/role", { role: item.role, store: item.id })
		        .then(res => {
		          	this.text = res.data.store.name + "'s Role Updated to " + res.data.store.role;
		          	this.snackbar = true;
		        })
		        .catch(error => {
		          	this.text = error.response.data.store.name + "'s Role Cannot Be Updated to " + error.response.data.store.role;
		          	this.store.data[index].role = error.response.data.store.role;
		          	this.snackbar = true;
		        });
	    	},
	    	selectAll(e) {
	    	...
	    	deleteAll() {
		        ...
		        if(decide) {
			        ...
			        //axios.post('/api/store/delete', {'store': this.selected})
			        axios.post('/api/store/delete', {'store': selected_id})
			        .then(res => {
		        		this.text = "Records Deleted Successfully!";
			        	this.selected.map(val => {
			        		const index = this.store.data.indexOf(val)
		    ...
		    editItem (item) {
		        this.editedIndex = this.store.data.indexOf(item)
		    ...
		    save () {
		        if (this.editedIndex > -1) {
		        	...
		        	axios.put('/api/store/' + this.editedItem.id, this.editedItem)
		        	.then(res => {
		        		this.text = "Record Updated Successfully!";
		        		this.snackbar = true;
		        		Object.assign(this.store.data[index], res.data.store)
		        	})
		        	.catch(err => {
		        		console.log(err.response)
			        ...
			        axios.post('/api/store', this.editedItem)
			    	.then(res => {
		        		this.text = "Record Added Successfully!";
		        		this.snackbar = true;
			    		this.store.data.push(res.data.store)
			    	})
			    	.catch(err => {
		        		console.log(err.response)
