<template>
	<v-app id="inspire">
	    ...
	    :headers="headers" :options.sync="options" :server-items-length="payment.total" :items="payment.data" show-select @input="selectAll" :footer-props="footerProps">
	        <template v-slot:top>
	            <v-toolbar flat>
    ...
    export default {
        data: () => ({
        	...
        	footerProps: {
				...
				itemsPerPageText: 'payment Per Page',
	    ...
	    watch: {
		    ...
		    options: {
		    	handler(e) {
					...
					axios.get(`/api/payment`, {params: {'page': e.page,'per_page': e.itemsPerPage, 'sort_by': sortBy, 'order_by': orderBy}})
					.then(res => {
	    ...
	    methods: {
	    	uploadPhoto(item) {
	    		if (this.editedItem.photo != null) {
			        const index = this.payment.data.indexOf(item);
			        let formData = new FormData();
			        formData.append( "photo", this.editedItem.photo, this.editedItem.photo.name );
			        formData.append("payment", item.id);
			        axios.post("/api/payment/photo", formData)
			    	.then(res => {
			    		this.payment.data[index].photo = res.data.payment.photo;
	    	...
	    	updateRole(item) {
	    		const index = this.payment.data.indexOf(item);
		      	axios.post("/api/payment/role", { role: item.role, payment: item.id })
		        .then(res => {
		          	this.text = res.data.payment.name + "'s Role Updated to " + res.data.payment.role;
		          	this.snackbar = true;
		        })
		        .catch(error => {
		          	this.text = error.response.data.payment.name + "'s Role Cannot Be Updated to " + error.response.data.payment.role;
		          	this.payment.data[index].role = error.response.data.payment.role;
		          	this.snackbar = true;
		        });
	    	},
	    	selectAll(e) {
	    	...
	    	deleteAll() {
		        ...
		        if(decide) {
			        ...
			        //axios.post('/api/payment/delete', {'payment': this.selected})
			        axios.post('/api/payment/delete', {'payment': selected_id})
			        .then(res => {
		        		this.text = "Records Deleted Successfully!";
			        	this.selected.map(val => {
			        		const index = this.payment.data.indexOf(val)
	    	...
	    	searchIt(e) {
	    		if(e.length > 2) {
	    			axios.get(`/api/payment/${e}`)
	    			.then(res => this.payment = res.data.payment)
		    ...
		    editItem (item) {
		        this.editedIndex = this.payment.data.indexOf(item)
		    ...
		    deleteItem (item) {
		        ...
		        if(decide) {
			        axios.delete('/api/payment/' + item.id)
			        .then(res => {
		    ...
		    save () {
		        if (this.editedIndex > -1) {
		        	...
		        	axios.put('/api/payment/' + this.editedItem.id, this.editedItem)
		        	.then(res => {
		        		this.text = "Record Updated Successfully!";
		        		this.snackbar = true;
		        		Object.assign(this.payment.data[index], res.data.payment)
		        	})
		        	.catch(err => {
		        		console.log(err.response)
			        ...
			        axios.post('/api/payment', this.editedItem)
			    	.then(res => {
		        		this.text = "Record Added Successfully!";
		        		this.snackbar = true;
			    		this.payment.data.push(res.data.payment)
			    	})
			    	.catch(err => {
		        		console.log(err.response)
