<template>
	<v-app id="inspire">
	    ...
	    :headers="headers" :options.sync="options" :server-items-length="reports.total" :items="reports.data" show-select @input="selectAll" :footer-props="footerProps">
	        <template v-slot:top>
	            <v-toolbar flat>
	                <v-toolbar-title >Report Management System</v-toolbar-title>
	                <v-divider class="mx-4" inset vertical></v-divider>
	                <v-spacer></v-spacer>
	                <v-dialog v-model="dialog" max-width="500px">
	                    <template v-slot:activator="{ on }">
				            <v-btn color="primary" dark class="mb-2" v-on="on">Order Report By</v-btn>
				            <!--v-list dense>
    ...
    export default {
        data: () => ({
        	...
        	footerProps: {
				...
				itemsPerPageText: 'Reports Per Page',
        ...
        computed: {
	      	formTitle () {
	        	return this.editedIndex === -1 ? 'Order Report' : 'Edit Report'
	      	},
	    },
	    watch: {
		    dialog (val) {
		    ...
		    order () {
			    axios.post('/api/reports', {'name': this.editedItem.name })
			   	.then(res => {
		        	...
		        	this.snackbar = true;
			    	this.reports.data.push(res.data.report)
			    })
			    .catch(err => {
